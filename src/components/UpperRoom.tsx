...
import {
  X, Send, MessageCircle, Loader2, Sparkles, Smile,
...
export default function UpperRoom({
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      ...
      if (diffDays <= 0) {
        setFastingPreaching("Preparation: Sanitize your heart. The fast begins on Jan 12th.")
      } else {
  ...
  useEffect(() => {
    if (currentRoom === 'private' && unreadSenders.length === 0) {
      setHasNewDM(false)
    }
  ...
  const renderTextWithMentions = (text: string) => {
    ...
    return parts.map((part, i) => 
      part.startsWith('@') 
        ? <span key={i} className="text-indigo-600 font-bold bg-indigo-50 rounded px-0.5">{part}</span> 
        : part
    )
  }
    ...
    <>
      {!isOpen && !isFullPage && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-indigo-600 text-white p-4 rounded-full shadow-2xl transition-all group hover:scale-110"
        >
      ...
      <dialog
        ...
        className="m-0 p-0 border-0 bg-transparent w-full max-w-md h-full ml-auto"
      >
        <div 
          className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[-1]" 
          onClick={() => !isFullPage && setIsOpen(false)}
        />

        <div className="relative bg-white flex flex-col h-[100dvh] w-full max-w-md ml-auto shadow-2xl animate-in slide-in-from-right duration-300">
          
          {/* Room Tabs */}
          <div className="bg-slate-950 px-2 pt-[env(safe-area-inset-top)] shrink-0">
            <div className="flex bg-white/5 p-1 rounded-xl gap-1">
              <button 
                onClick={() => setCurrentRoom('general')}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${currentRoom === 'general' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}
              >
                <Globe size={14} /> Fellowship
              </button>
              <button 
                onClick={() => setCurrentRoom('fasting')}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${currentRoom === 'fasting' ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}
              >
                <Flame size={14} /> The Altar
              </button>
              <button 
                onClick={() => setCurrentRoom('private')}
                className={`relative flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${currentRoom === 'private' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5'}`}
              >
                <Mail size={14} /> DMs
          ...
          <div className="bg-slate-900 p-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              {currentRoom === 'private' && selectedRecipient && (
                <button onClick={() => setSelectedRecipient(null)} className="text-white bg-white/10 p-1.5 rounded-lg hover:bg-white/20">
                  <X size={14}/>
                </button>
              )}
              <h3 className="text-white font-black italic uppercase text-sm flex items-center gap-2">
                {currentRoom === 'private' && selectedRecipient ? (
                  <span className="flex items-center gap-2 text-emerald-400">
                    <div className="w-5 h-5 rounded-full bg-emerald-500 text-white text-[8px] flex items-center justify-center font-black">
              ...
              <div className="absolute inset-0 bg-white z-[110] flex flex-col">
                <div className="p-4 border-b border-slate-100 bg-slate-50">
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                      value={searchTerm}
                ...
                <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2">
                  {unreadSenders.length > 0 && !searchTerm && (
                    <div className="mb-6">
                      <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">New Messages</p>
                      {allUsers.filter(u => unreadSenders.includes(u.id)).map(u => (
                        <button 
                          ...
                          className="w-full flex items-center justify-between p-4 rounded-2xl bg-emerald-50 border border-emerald-200 shadow-sm animate-pulse mb-2 text-left"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="font-bold text-emerald-900 block text-sm">{u.full_name}</span>
                              <span className="text-[10px] text-emerald-600 font-bold uppercase tracking-tight">Click to read message</span>
                            </div>
                          </div>
                  ...
                  {filteredDirectory.filter(u => !unreadSenders.includes(u.id)).map(u => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-black text-slate-500 group-hover:bg-emerald-500 group-hover:text-white">
                          {u.full_name.charAt(0)}
                        </div>
                  ...
                  return (
                    <div key={index} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} group animate-in fade-in slide-in-from-bottom-1`}>
                      <div className={`flex items-end gap-2 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        <div className={`relative rounded-2xl text-sm shadow-sm transition-all ${isMe ? 'bg-indigo-100 text-indigo-900 rounded-br-none pl-8 pr-4 py-3' : 'bg-white text-slate-800 border border-slate-200 rounded-bl-none pr-8 pl-4 py-3'}`}>
                          {!isMe && <p className="text-[9px] font-black text-indigo-500 uppercase mb-1">{msg.author_name}</p>}

                          {msg.reply_to && (
                            <div className={`mb-2 p-2 rounded-lg text-xs border-l-2 ${isMe ? 'bg-indigo-50 border-indigo-200 text-indigo-900' : 'bg-slate-100 border-slate-300 text-slate-500'}`}>
                          ...
                          {isEditing ? (
                            <div className="flex flex-col gap-2 min-w-[200px]">
                              <textarea 
                                ...
                                className="w-full text-slate-800 bg-white rounded p-2 text-sm outline-none ring-2 ring-orange-500" 
                                rows={2}
                              />
                              <div className="flex justify-end gap-2">
                                <button onClick={() => setEditingId(null)} className="text-xs">Cancel</button>
                                <button onClick={() => saveEdit(msg.id)} className="bg-white text-indigo-600 px-2 py-1 rounded text-xs font-bold flex items-center gap-1">
                                  <Check size={12}/> Save
                                </button>
                              </div>
                            </div>
                          ...
                          {!isEditing && (
                            <div className={`absolute top-2 z-10 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity ${isMe ? 'left-2' : 'right-2'}`}>
                              <button 
                                ...
                                className={`p-1 hover:bg-black/20 rounded-full transition-colors ${isMe ? 'text-indigo-500/60 hover:text-indigo-900' : 'text-slate-300 hover:text-indigo-500'}`}
                              >
                              ...
                              {activeMenuId === msg.id && (
                                <div className={`absolute top-6 bg-white shadow-xl rounded-xl border border-slate-100 z-50 w-36 flex flex-col animate-in fade-in zoom-in duration-200 ${isMe ? 'left-0' : 'right-0'}`}>
                                  ...
                                  {!isMe && (
                                    <button onClick={() => handleStartPrivateChat(msg.user_id, msg.author_name)} className="text-left px-3 py-2 text-xs text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 flex items-center gap-2 border-t border-slate-50">
                                      <Mail size={10} /> Message Privately
                                    </button>
                                  )}
                                  {isMe && msg.type === 'text' && (
          ...
          {/* Input area */}
          <div className={`p-4 bg-white border-t relative pb-[env(safe-area-inset-bottom)]`}>
              ...
              <div className="absolute bottom-20 left-4 bg-white shadow-2xl rounded-2xl border border-slate-200 z-[120] w-64 max-h-48 overflow-y-auto">
                ...
                {filteredUsers.map(u => (
                  <button 
                    ...
                    className="w-full text-left px-4 py-3 text-sm hover:bg-slate-50 flex items-center gap-2 border-b border-slate-50"
                  >
            ...
            {isRecording ? (
              ...
              <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
                <button 
                  ...
                  className="p-3 rounded-xl border bg-slate-50 text-slate-400 h-11 hover:text-indigo-500 transition-colors"
                >
                  <Smile size={20} />
                </button>
                <textarea
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-indigo-500/20 outline-none resize-none max-h-32"
                  rows={1}
                />
                {newMessage.trim() ? (
                  <button 
                    disabled={isSending} 
                    className={`${currentRoom === 'fasting' ? 'bg-orange-600' : currentRoom === 'private' ? 'bg-emerald-600' : 'bg-indigo-600'} text-white p-3 rounded-xl h-11 transition-all active:scale-95`}
                  >
                  ...
                  <button 
                    ...
                    className="bg-orange-500 text-white p-3 rounded-xl h-11 active:scale-95 transition-all"
                  >
                    <Mic size={20} />
                  </button>
                )}
