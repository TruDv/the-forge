...
export default function UpperRoom({ user, profileName, isFullPage = false }: { user: any, profileName: string, isFullPage?: boolean }) {
  ...
  return (
    <>
      {!isOpen && !isFullPage && (
        <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-50 bg-indigo-600 text-white p-4 rounded-full shadow-2xl transition-all group hover:scale-110">
            ...
            <div className={`p-4 bg-white border-t relative shrink-0 ${isFullPage ? 'pb-24' : 'pb-safe'}`}>
              ...
              {isRecording ? (
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2 items-end">
                  ...
                  />
                  {newMessage.trim() ? (
                    <button disabled={isSending} className={`${currentRoom === 'fasting' ? 'bg-orange-600' : currentRoom === 'private' ? 'bg-emerald-600' : 'bg-indigo-600'} text-white p-3 rounded-xl h-11 transition-all active:scale-95`}>
