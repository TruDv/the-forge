...
export default function AdminPortal() {
  ...
  const postOracleCharge = async () => {
    ...
    const { error } = await supabase.from('announcements').insert([{ 
      ...
      link: newLink.trim() || null // <--- Save the link
    }]);

    if (!error) {
      setNewCharge(''); 
      setNewLink(''); // Reset link input
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-6 text-slate-900">
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* CENTER COLUMN: THE ORACLE (UPDATED FOR LINK) */}
          ...
          <div className="lg:col-span-1 bg-white rounded-[32px] border border-slate-100 shadow-sm flex flex-col overflow-hidden max-h-[850px]">
             <div className="flex border-b border-slate-50 bg-slate-50/50 p-1">
               <button onClick={() => setActiveTab('prayers')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'prayers' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}><MessageSquare size={14} /> Prayers</button>
               <button onClick={() => setActiveTab('archives')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'archives' ? 'bg-white shadow-sm text-orange-600' : 'text-slate-400'}`}><History size={14} /> Oracle</button>
               <button onClick={() => setActiveTab('media')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'media' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}><Video size={14} /> Media</button>
               <button onClick={() => setActiveTab('users')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'users' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}><Users size={14} /> People</button>
             </div>
             ...
             <div className="overflow-y-auto p-4 space-y-2 flex-1">
               {isLoading ? (
                 ...
                 <>
                   ...
                   {/* ARCHIVES TAB (Updated to show link icon) */}
                   ...
                   {activeTab === 'media' && (
                     <div className="space-y-4">
                       <button onClick={handleCreateMedia} className="w-full border-2 border-dashed border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 py-4 rounded-2xl text-slate-400 hover:text-indigo-600 font-bold text-xs uppercase flex items-center justify-center gap-2 transition-all"><Plus size={16} /> Upload New Video</button>
                       {mediaList.map((m) => (
                         <div key={m.id} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center group border border-slate-100">
                           <div className="flex items-center gap-3 overflow-hidden">
                             <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 text-white shadow-md ${m.type === 'video' ? 'bg-gradient-to-br from-indigo-500 to-purple-600' : 'bg-gradient-to-br from-orange-400 to-orange-600'}`}>{m.type === 'video' ? <Video size={16}/> : <Mic size={16}/>}</div>
                             <div className="flex-1 min-w-0"><p className="text-xs font-bold text-slate-900 truncate">{m.title}</p><p className="text-[10px] text-slate-500 truncate">{m.author || "The Forge"}</p></div>
                           </div>
                           <div className="flex gap-1"><button onClick={() => handleEditMedia(m)} className="text-slate-300 hover:text-indigo-600 p-2 transition-colors"><Edit2 size={16}/></button><button onClick={() => deleteItem('media', m.id)} className="text-slate-300 hover:text-rose-600 p-2 transition-colors"><Trash2 size={16}/></button></div>
                         </div>
                       ))}
                     </div>
                   )}
                   ...
                   {activeTab === 'users' && (
                     <div className="space-y-4">
                        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={14} /><input placeholder="Search Name or Email..." className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs outline-none focus:border-indigo-300 transition-colors" value={userSearch} onChange={(e) => setUserSearch(e.target.value)} /></div>
                        {filteredUsers.length > 0 ? (
                          filteredUsers.map((u) => (
                             <div key={u.id} className={`p-4 rounded-2xl flex justify-between items-center group border transition-all ${u.is_blocked ? 'bg-rose-50 border-rose-100 opacity-75' : 'bg-slate-50 border-slate-100'}`}>
                               <div className="flex items-center gap-3 overflow-hidden">
                                 <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black uppercase flex-shrink-0 ${u.is_blocked ? 'bg-rose-200 text-rose-700' : 'bg-indigo-100 text-indigo-700'}`}>{u.full_name?.charAt(0) || '?'}</div>
                                 <div className="flex-1 min-w-0"><p className={`text-xs font-bold truncate ${u.is_blocked ? 'text-rose-900 line-through' : 'text-slate-900'}`}>{u.full_name || 'Unnamed'}</p><p className="text-[9px] text-slate-500 truncate">{u.email || 'No Email'}</p></div>
                               </div>
                               <div className="flex gap-1"><button onClick={() => toggleUserBlock(u.id, u.is_blocked)} className={`p-2 transition-colors rounded-lg ${u.is_blocked ? 'text-emerald-500 hover:bg-emerald-100' : 'text-slate-300 hover:text-orange-500 hover:bg-orange-50'}`}>{u.is_blocked ? <CheckCircle size={16}/> : <Ban size={16}/>}</button><button onClick={() => hardDeleteUser(u.id)} className="text-slate-300 hover:text-rose-600 hover:bg-rose-50 p-2 rounded-lg transition-colors"><Trash2 size={16}/></button></div>
                             </div>
                          ))
